...
<div x-data="konsultasiHandler()" class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-gradient-to-br from-indigo-700 to-indigo-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
        <div class="relative z-10 space-y-3">
            <h2 class="text-3xl font-bold italic">Tazkiyatun Nafs <span class="text-amber-400">& Bimbingan Spiritual</span></h2>
            <p class="text-indigo-100/90 text-sm max-w-lg leading-relaxed">Berikan curhatan atau masalah pribadi Anda, dan dapatkan bimbingan spiritual serta amalan penyejuk hati yang bersumber dari ajaran Islam.</p>
        </div>
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 space-y-6">
        ...
        <div class="relative group">
            <textarea 
                ...
                placeholder="Tuliskan apa yang mengganjal di hati Anda atau masalah yang sedang dihadapi..."></textarea>
        ...
        <button @click="solveProblem()" 
            ...
            <template x-if="!loading">
                <span class="flex items-center gap-2">🌱 Dapatkan Bimbingan</span>
            </template>
            <template x-if="loading">
                <div class="flex items-center gap-3">
                    <div class="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin"></div>
                    <span>AI sedang menelaah hati...</span>
                </div>
            </template>
        </button>
    </div>
    ...
    <div x-show="advice || loading" 
        ...
        <div class="px-8 py-4 border-b border-slate-50 flex items-center justify-between bg-amber-50/20">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-black text-amber-600 uppercase tracking-widest">Bimbingan Spiritual</h3>
            </div>
